...
1. **能力点**：只允许从“毕业要求指标点”相关表格中提取能力点，每个能力点仅保留描述文本
2. **评价标准**：针对每个能力点自动生成3条可执行、可判定、可量化的评价标准
...
- 分析结果必须详细、具体，避免泛泛而谈
- 能力点只能来自“毕业要求指标点”表格原文，不得从其他章节补充
- 优先识别表头为“序号 / 毕业要求指标点 / 毕业要求指标点内容”的表格，能力点内容必须来自“毕业要求指标点内容”列
- 不得把“课程教学目标与毕业要求对应关系、课程内容/教学要求/学时分配、课程立德树人内涵”等章节标题当作能力点
- 每个能力点只保留 `description` 字段，不输出掌握程度等级
- 评价标准必须按能力点逐一生成，每个能力点固定3条标准
- 三条标准必须体现递进逻辑：基础理解 -> 应用实施 -> 分析改进
- 每条标准必须包含评价对象、达成条件、判定依据（可观察行为/质量要求/达成阈值）
- 禁止机械复用同一句式，避免“模板化套话”，语言要自然、专业且与该能力点语义强相关
...
{{
    "ability_points": [
        {{
            "description": "毕业要求指标点表格中的原始或等价改写描述"
        }}
    ],
    "evaluation_criteria": [
        {{
            "ability_description": "对应的能力点描述",
            "standards": [
                "标准1",
                "标准2",
                "标准3"
...
1. **能力点**：只允许从“毕业要求指标点”相关表格中提取，每个能力点仅保留描述
2. **评价标准**：针对每个能力点自动生成3条评价标准
...
- 分析结果必须详细、具体，避免泛泛而谈
- 能力点只能来自“毕业要求指标点”表格内容；非表格内容不得提取为能力点
- 优先识别表头为“序号 / 毕业要求指标点 / 毕业要求指标点内容”的表格，能力点内容必须来自“毕业要求指标点内容”列
- 不得把“课程教学目标与毕业要求对应关系、课程内容/教学要求/学时分配、课程立德树人内涵”等章节标题当作能力点
- 每个能力点仅输出 `description`，不输出 `name` / `level`
- 评价标准按能力点逐一生成，每个能力点固定3条标准
- 三条标准必须体现递进逻辑：基础理解 -> 应用实施 -> 分析改进
- 每条标准必须包含评价对象、达成条件、判定依据（可观察行为/质量要求/达成阈值）
- 禁止机械复用同一句式，避免“模板化套话”，语言要自然、专业且与该能力点语义强相关
- 评价标准要可执行、可判定、可量化，可基于专业语境做合理扩展
- 毕业要求指标点需要具体到指标点编号和具体要求
