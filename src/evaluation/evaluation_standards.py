...
{{
    ...
    "dimension_scores": [
        {{
            ...
            "score": 85,
            "grade_level": "良好",
            "evidence": ["具体证据1（引用原文）", "具体证据2（引用原文）"],
            "reasoning": "详细评分理由（100-200字，必须引用具体内容，说明为什么给出这个分数，哪些内容加分，哪些内容扣分，与评分标准的差距在哪里）",
            "strengths": ["该维度的优点1（附证据）", "该维度的优点2（附证据）"],
            "weaknesses": ["该维度的不足1（附证据）", "该维度的不足2（附证据）"],
            "improvement_suggestions": ["具体可操作的改进建议1", "具体可操作的改进建议2"]
        }}
    ],
    "strengths": ["优势1（附具体证据）", "优势2（附具体证据）", "优势3（附具体证据）"],
    "weaknesses": ["劣势1（附具体证据）", "劣势2（附具体证据）"],
    "improvement_suggestions": [
        {{"aspect": "改进方面", "current_issue": "当前问题", "suggestion": "具体建议", "priority": "高/中/低"}}
    ],
    "overall_evaluation": "总体评价（300字以上，需包含：1.论文整体质量概述 2.主要优点及证据 3.主要不足及证据 4.与同类毕业设计相比的水平 5.改进方向建议）",
    "detailed_reasoning": "各维度评分的详细推理过程（200-300字）：综合分析各维度的得分逻辑，说明总分是如何得出的，哪些维度是主要加分项，哪些是主要扣分项"
}}
"""
    return prompt
