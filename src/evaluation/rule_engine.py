...
<<<<<<< HEAD
            indicator_id = indicator.get("indicator_id") or indicator.get("id", f"IND_{idx:02d}")
=======
...
<<<<<<< HEAD
        elif any(kw in name_lower for kw in ["文献", "literature", "调研", "综述"]):
            rules.extend(self._generate_literature_rules(indicator_id, indicator, weight))
        
        elif any(kw in name_lower for kw in ["沟通", "communication", "表达", "交流"]):
            rules.extend(self._generate_communication_rules(indicator_id, indicator, weight))
        
        elif any(kw in name_lower for kw in ["国际", "international", "视野", "跨文化"]):
            rules.extend(self._generate_international_rules(indicator_id, indicator, weight))
        
        elif any(kw in name_lower for kw in ["需求", "requirement", "针对"]):
            rules.extend(self._generate_requirement_rules(indicator_id, indicator, weight))
        
        elif any(kw in name_lower for kw in ["权衡", "trade-off", "制约", "因素"]):
            rules.extend(self._generate_tradeoff_rules(indicator_id, indicator, weight))
...
<<<<<<< HEAD
    def _generate_literature_rules(self, indicator_id: str, indicator: Dict, weight: float) -> List[ScoringRule]:
        """生成文献分析相关规则"""
        rules = []
        
        literature_keywords = [
            "文献", "参考文献", "引用", "综述", "调研", "研究现状",
            "国内外", "研究进展", "相关工作", "背景", "文献综述"
        ]
        
        rules.append(ScoringRule(
            rule_id=f"{indicator_id}_R01",
            rule_type=RuleType.KEYWORD_MATCH,
            description="文献分析关键词覆盖度",
            weight=weight * 0.4,
            parameters={
        ...
        rules.append(ScoringRule(
            rule_id=f"{indicator_id}_R02",
            rule_type=RuleType.STRUCTURE_CHECK,
            description="文献综述章节完整性",
            weight=weight * 0.3,
            parameters={
                "required_sections": [
                    "文献综述", "研究现状", "国内外研究", "相关工作",
                    "背景介绍", "文献分析", "理论基础"
                ],
                "min_sections": 2
            },
    ...
    def _generate_communication_rules(self, indicator_id: str, indicator: Dict, weight: float) -> List[ScoringRule]:
        """生成专业沟通相关规则"""
        rules = []
    ...
    def _generate_international_rules(self, indicator_id: str, indicator: Dict, weight: float) -> List[ScoringRule]:
        """生成国际化视野相关规则"""
        rules = []
        
        international_keywords = [
            "国际", "国外", "海外", "英文", "外语", "跨文化",
            "global", "international", "foreign", "英语", "外文"
        ]
    ...
    def _generate_requirement_rules(self, indicator_id: str, indicator: Dict, weight: float) -> List[ScoringRule]:
        """生成需求设计相关规则"""
        rules = []
    ...
    def _generate_tradeoff_rules(self, indicator_id: str, indicator: Dict, weight: float) -> List[ScoringRule]:
        """生成权衡分析相关规则"""
        rules = []
...
<<<<<<< HEAD
            indicator_scores[indicator_id]["rule_details"].append({
                "rule_id": rule.rule_id,
