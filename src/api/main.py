...
from src.database.models import (
    Student, Submission, MediaFile, EvaluationResult, ProgressReport,
...
def _cluster_feedback_texts(responses: List[Any]) -> List[Dict[str, Any]]:
    ...
    results = []
    for theme, item in clusters.items():
...
def _build_feedback_analytics(survey: Any, responses: List[Any], db_service: DatabaseService) -> Dict[str, Any]:
    ...
    for response in responses:
        sentiment_counts[getattr(response, "sentiment", None) or "neutral"] = sentiment_counts.get(getattr(response, "sentiment", None) or "neutral", 0) + 1
    ...
    clusters = _cluster_feedback_texts(responses)
    return {
...
async def list_feedback_surveys(
    ...
    for survey in surveys:
        count = len(db_service.get_feedback_responses(survey.survey_id))
        responses.append(_feedback_survey_response(survey, count))
    return responses
...
async def update_feedback_survey(
    ...
    survey = db_service.update_feedback_survey(survey_id, **payload)
    if not survey:
        raise HTTPException(status_code=404, detail="反馈问卷不存在")
    count = len(db_service.get_feedback_responses(survey.survey_id))
    return _feedback_survey_response(survey, count)
...
async def get_feedback_survey_analytics(
    ...
    if not survey:
        raise HTTPException(status_code=404, detail="反馈问卷不存在")
    responses = db_service.get_feedback_responses(survey_id)
    return _build_feedback_analytics(survey, responses, db_service)
...
async def list_calibration_benchmarks(
    ...
    _ = auth
    benchmarks = db_service.get_calibration_benchmarks(skip=skip, limit=limit)
    return [_benchmark_response(item, db_service) for item in benchmarks]
...
async def list_calibration_reports(
    ...
    _ = auth
    reports = db_service.get_calibration_reports(benchmark_id=benchmark_id)
    payloads = []
